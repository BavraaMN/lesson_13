  ...
  const game=(language)  => {
   ...
   const langQUESTIONS= language==='EN' || language==='ENG' ? QUESTIONS_ENG:QUESTIONS_RUS;
    return function start() {
      ...
      let selectWord = prompt("Обязательное задание чего вы выберите камень(к..) ножница(н..) бумаг(б..) ?", "");
        ...
        //  alert('ок ножницы');
         ...
         let selectedtheirindex=getRandomIntInclusive(0,2);                
          if (selectedtheirindex==selectedOurIndex)
            {
               alert( 'Computer выбрал '+FIGURES_RUS[selectedtheirindex]+ " Вы выбрали "+FIGURES_RUS[selectedOurIndex]+ ' Ничье');
            }
            else   if ((selectedOurIndex+1==selectedtheirindex) || ((selectedtheirindex==0) && (selectedOurIndex==2)))
             { 
                 alert('Computer выбрал '+FIGURES_RUS[selectedtheirindex]+ " Вы выбрали "+FIGURES_RUS[selectedOurIndex] +' Вы выграли');
                 result.player++;
             }
             else  if ((selectedOurIndex==selectedtheirindex+1) || ((selectedtheirindex==2) && (selectedOurIndex==0)))
               {
                   alert('Computer выбрал '+FIGURES_RUS[selectedtheirindex]+ " Вы выбрали "+FIGURES_RUS[selectedOurIndex]+ ' Сomputer выграл');
                   result.computer++;
               } 
             ...
             if (confirm('Закончить игру'))
             {
                if (confirm("Вы уверены"))
                 {
                  alert('ЧЕЛОВЕК '+humanwin+' ВЫГРАЛ '+'\n КОМПЬЮТОР '+compwin+' выграл');  
                  result.player=0;
                  result.computer=0;
                  return;  
                 } 
